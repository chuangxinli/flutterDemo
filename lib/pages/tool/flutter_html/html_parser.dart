  ...
  Widget _parseNode(dom.Node node) {
    ...
    if (node is dom.Element) {
      ...
      switch (node.localName) {
        ...
        case "span":
          int _textStyleType = 0; //0 正常，1，有下划线， 2，有波浪下
          if (node.attributes['class'] != null &&
              node.attributes['class'].contains('underline')) {
            _textStyleType = 1;
          } else if (node.attributes['class'] != null &&
              node.attributes['class'].contains('wavy')) {
            _textStyleType = 2;
          } else if (node.attributes['class'] != null &&
              node.attributes['class'].contains('singleDot')) {
            _textStyleType = 3;
          }
          if (_textStyleType == 0) {
            return Wrap(
          ...
          } else if (_textStyleType == 1 || _textStyleType == 2) {
            return DefaultTextStyle.merge(
              child: Wrap(
                children: _parseNodeList(node.nodes),
              ),
              style: TextStyle(
                  decoration: TextDecoration.underline,
                  decorationStyle: _textStyleType == 1
          ...
          children: <Widget>[
            ...
            Positioned(
              top: new RegExp(r"\w").hasMatch(node.text) ? 6 : 9,
              child: Text(
                '·',
                style: TextStyle(
                    fontSize: 14,
        ...
        case "table":
          List<dom.Element> eleList = node.querySelectorAll('*');
          bool isExistColspan = false;
          for (int i = 0; i < eleList.length; i++) {
            if (eleList[i].attributes['colspan'] != null) {
              isExistColspan = true;
              break;
            }
          }
          if (!isExistColspan) {
            List<dom.Element> thList = node.querySelectorAll('th');
            List<dom.Element> trList = node.querySelectorAll('tr');
            thList.addAll(trList);
            List<Widget> getTdWidget(dom.Element node) {
              List<dom.Element> tdList = node.querySelectorAll('td');
              List list = <Widget>[];
              for (int i = 0; i < tdList.length; i++) {
                list.add(Center(
                  child: Wrap(
                      crossAxisAlignment: WrapCrossAlignment.center,
                      children: _parseNodeList(tdList[i].nodes)),
                ));
              }
              return list;
            }
            ...
            List<TableRow> getTableRow() {
              ...
              for (int i = 0; i < thList.length; i++) {
                list.add(TableRow(children: getTdWidget(thList[i])));
              }
              return list;
            }
